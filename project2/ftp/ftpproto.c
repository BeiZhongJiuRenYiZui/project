...
{
    //send(sess->ctrl_fd,"220 (miniftp 1.o.1)\r\n", strlen("220 (miniftp 1.0.1)\r\n"), 0);
    ftp_reply(sess, FTP_GREET, "(ftp miniftp 1.0.1)");
    while(1)
    {
        //不停的等待客户端的命令并做出处理
...
static void do_user(session_t *sess)
{
    ...
    if(pwd != NULL)
        sess->uid = pwd->pw_uid; //保存用户ID及uid
...
static void do_type(session_t *sess)
{
    ...
    {
        sess->is_ascii = 1;
        ftp_reply(sess, FTP_TYPEOK, "Switching to ASSII mode.");
    }
